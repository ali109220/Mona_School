...
namespace Application.Startup
{
    ...
    builder =>
    {
        ...
        builder.WithOrigins("https://cxe.ebk.mybluehost.me", "https://cxe.ebk.mybluehost.me/mona")
               .AllowAnyHeader().AllowAnyMethod();
    });
            });

            services.AddDbContext<MonaContext>(options =>
                options.UseSqlServer(Configuration.GetConnectionString("DefaultConnection")));

            services.AddIdentity<User, IdentityRole>()
                .AddEntityFrameworkStores<MonaContext>()
            ...
            services.AddTransient<IMonaBackgrounJobService, MonaBackgrounJobService>();
        }

        // This method gets called by the runtime. Use this method to configure the HTTP request pipeline.
        public void Configure(IApplicationBuilder app,IBackgroundJobClient backgroundJobs, IServiceProvider serviceProvider, IHostingEnvironment env)
        {
            if (env.IsDevelopment())
            {
            ...
            RecurringJob.AddOrUpdate(
                () => serviceProvider.GetService<IMonaBackgrounJobService>().UpdateObjStatus(),
                "*/2 * * * *"
                //serviceProvider.GetService<ICourseService>().ChangeStatus(),
                );
            RecurringJob.AddOrUpdate(
                () => serviceProvider.GetService<IMonaBackgrounJobService>().SendNextLessonEmail(),
                "*/5 * * * *"
                //serviceProvider.GetService<ICourseService>().ChangeStatus(),
                );
